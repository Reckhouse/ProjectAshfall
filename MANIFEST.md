# Package Manifest

## Root
- `README.md`
- `CURSOR_START_HERE.md`

## Architecture and design
- `docs/00_DECISIONS_AND_ASSUMPTIONS.md`
- `docs/01_GAME_VISION_AND_LOOP.md`
- `docs/02_GAME_STATE_AND_COMMAND_MODEL.md`
- `docs/03_TECHNICAL_ARCHITECTURE.md`
- `docs/04_PHASE_1_AUTH_AND_BASE_SPAWN.md`
- `docs/05_WORLD_GRID_AND_GENERATION.md`
- `docs/06_DATABASE_SCHEMA.md`
- `docs/07_RESOURCES_AND_TOOLS.md`
- `docs/08_CAVES_AND_LOOT.md`
- `docs/09_BASE_TROOPS_AND_COMBAT.md`
- `docs/10_API_AND_DOMAIN_CONTRACTS.md`
- `docs/11_SECURITY_AND_ANTI_CHEAT.md`
- `docs/12_TESTING_AND_OBSERVABILITY.md`
- `docs/13_PHASE_ROADMAP.md`
- `docs/14_UI_UX_DIRECTION.md`
- `docs/15_PERFORMANCE_AND_SCALING.md`
- `docs/16_RNG_AND_REPRODUCIBILITY.md`
- `docs/17_DEPLOYMENT_VERCEL_NEON.md`
- `docs/18_CURSOR_WORKFLOW.md`

## Balance
- `balance/BALANCE_BASELINE.md`
- `balance/BALANCE_METRICS.md`
- `balance/ECONOMY_BALANCE.md`
- `balance/SPAWN_AND_WORLD_BALANCE.md`
- `balance/TOOL_AND_CAVE_BALANCE.md`
- `balance/COMBAT_BALANCE.md`
- `balance/SIMULATION_PLAN.md`
- `balance/TELEMETRY_SPEC.md`
- `balance/AGENT_HANDOFF_PROTOCOL.md`

## Cursor agents
- `cursor_agents/00_ASHFALL_ORCHESTRATOR_AGENT.md`
- `cursor_agents/01_GAME_DESIGN_GUARDIAN_AGENT.md`
- `cursor_agents/02_ECONOMY_BALANCE_AGENT.md`
- `cursor_agents/03_COMBAT_BALANCE_AGENT.md`
- `cursor_agents/04_WORLD_GENERATION_AGENT.md`
- `cursor_agents/05_PROGRESSION_BALANCE_AGENT.md`
- `cursor_agents/06_SECURITY_ANTI_CHEAT_AGENT.md`
- `cursor_agents/07_DATABASE_CONCURRENCY_AGENT.md`
- `cursor_agents/08_TEST_QA_AGENT.md`
- `cursor_agents/09_PHASE_1_IMPLEMENTATION_AGENT.md`
- `cursor_agents/10_FRONTEND_UI_AGENT.md`
- `cursor_agents/11_TELEMETRY_ANALYSIS_AGENT.md`

## Task packets
- `task_packets/PHASE_0_FOUNDATION.md`
- `task_packets/PHASE_1_TASKS.md`
- `task_packets/PHASE_2_GRID_WORLD.md`
- `task_packets/PHASE_3_RESOURCES.md`
- `task_packets/PHASE_4_CAVES_TOOLS.md`
- `task_packets/PHASE_5_TROOPS_EXPEDITIONS.md`
- `task_packets/PHASE_6_COMBAT_PVP.md`
